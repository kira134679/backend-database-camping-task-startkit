...
VALUES
    (
        (SELECT DISTINCT u.id  FROM "USER" u WHERE u.name = '王小明'),
        (SELECT DISTINCT cp.id FROM "CREDIT_PACKAGE" cp WHERE cp.name = '14 堂組合包方案'),
        (SELECT DISTINCT cp.credit_amount FROM "CREDIT_PACKAGE" cp WHERE cp.name = '14 堂組合包方案'),
        (SELECT DISTINCT cp.price FROM "CREDIT_PACKAGE" cp WHERE cp.name = '14 堂組合包方案')
    ),
    (
        (SELECT DISTINCT u.id  FROM "USER" u WHERE u.name = '王小明'),
        (SELECT DISTINCT cp.id FROM "CREDIT_PACKAGE" cp WHERE cp.name = '21 堂組合包方案'),
        (SELECT DISTINCT cp.credit_amount FROM "CREDIT_PACKAGE" cp WHERE cp.name = '21 堂組合包方案'),
        (SELECT DISTINCT cp.price FROM "CREDIT_PACKAGE" cp WHERE cp.name = '21 堂組合包方案')
    ),
    (
        (SELECT DISTINCT u.id  FROM "USER" u WHERE u.name = '好野人'),
        (SELECT DISTINCT cp.id FROM "CREDIT_PACKAGE" cp WHERE cp.name = '14 堂組合包方案'),
        (SELECT DISTINCT cp.credit_amount FROM "CREDIT_PACKAGE" cp WHERE cp.name = '14 堂組合包方案'),
        (SELECT DISTINCT cp.price FROM "CREDIT_PACKAGE" cp WHERE cp.name = '14 堂組合包方案')
    );
...
FROM
    "COACH" co
        CROSS JOIN
    (SELECT id FROM "SKILL" WHERE name = '重訓') sk;
...
WHERE
    user_id = (SELECT u.id FROM "USER" u WHERE u.email = 'muscle@hexschooltest.io');

UPDATE "COACH"
SET experience_years = 5
WHERE
    user_id = (SELECT u.id FROM "USER" u WHERE u.email = 'starplatinum@hexschooltest.io');
...
VALUES
    (
        ...
        (SELECT c.id FROM "COURSE" c, "USER" u WHERE c.user_id = u.id AND u.email = 'lee2000@hexschooltest.io'),
        '2024-11-24 16:00:00',
        '即將授課'
    );
    -- 2. 新增： `好野人` 預約 `李燕容` 的課程
...
AND
    course_id = (SELECT c.id FROM "COURSE" c, "USER" u WHERE c.user_id = u.id AND u.email = 'lee2000@hexschooltest.io');
...
SELECT
    user_id,
    COUNT(status NOT IN ('課程已取消', '即將授課')) total
FROM
    "COURSE_BOOKING"
WHERE
    user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io')
...
SELECT
    cp.user_id,
    (purchased_amount - consumed_amount) remaining_credit
FROM
    (
        SELECT user_id, SUM(purchased_credits) purchased_amount
        FROM "CREDIT_PURCHASE"
        GROUP BY user_id
    ) cp
INNER JOIN
    (
        SELECT user_id, COUNT(*) consumed_amount
        FROM "COURSE_BOOKING"
        WHERE status != '課程已取消'
        GROUP BY user_id
    ) cb
...
SELECT
    u.name 教練名稱,
    co.experience_years 經驗年數,
    sk.name 專長名稱
FROM "COACH_LINK_SKILL" cls
INNER JOIN "SKILL" sk ON cls.skill_id = sk.id
INNER JOIN "COACH" co ON cls.coach_id = co.id
INNER JOIN "USER" u ON co.user_id = u.id
WHERE sk.name = '重訓'
ORDER BY co.experience_years DESC;

-- 6-2 查詢：查詢每種專長的教練數量，並只列出教練數量最多的專長（需使用 group by, inner join 與 order by 與 limit 語法）
-- 顯示須包含以下欄位： 專長名稱, coach_total
SELECT
    sk.name,
    COUNT(cls.coach_id) coach_total
FROM "COACH_LINK_SKILL" cls
INNER JOIN "SKILL" sk ON cls.skill_id = sk.id
GROUP BY sk.id
